        ...
        if form.is_valid():
            if not request.session.test_cookie_worked():
                ...
                if error_code == 0:
                    ...
                    return redirect_to_prev_page(request, '/task/')
                elif error_code == 1:
                    error_message = u'User not exists, check username'
                elif error_code == 2:
                    error_message = u'Wrong password, enter again'
        else:
            error_message = u'input error'
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if user.password == form.cleaned_data['cur_password']:
                ...
                error_message = 'Original password error'
        else:
            error_message = form.errors
